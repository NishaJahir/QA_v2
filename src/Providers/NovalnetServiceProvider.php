...
use Novalnet\Methods\NovalnetSepa;
use Novalnet\Methods\NovalnetCc;
use Novalnet\Methods\NovalnetInvoice;
use Novalnet\Methods\NovalnetPaypal;
...
class NovalnetServiceProvider extends ServiceProvider
{
                          ...
                          EventProceduresService $eventProceduresService)
    {
        ...
        $payContainer->register('plenty_novalnet::NOVALNET_SEPA', NovalnetSepa::class,
            [
                AfterBasketChanged::class,
                AfterBasketItemAdd::class,
                AfterBasketCreate::class
            ]);
        $payContainer->register('plenty_novalnet::NOVALNET_CC', NovalnetCc::class,
            [
                AfterBasketChanged::class,
                AfterBasketItemAdd::class,
                AfterBasketCreate::class
            ]);
        $payContainer->register('plenty_novalnet::NOVALNET_INVOICE', NovalnetInvoice::class,
            [
                AfterBasketChanged::class,
                AfterBasketItemAdd::class,
                AfterBasketCreate::class
            ]);
        $payContainer->register('plenty_novalnet::NOVALNET_PAYPAL', NovalnetPaypal::class,
            [
                AfterBasketChanged::class,
                AfterBasketItemAdd::class,
                AfterBasketCreate::class
                ...
                function(GetPaymentMethodContent $event) use($paymentHelper, $paymentService, $addressRepository,  $basketRepository, $sessionStorage, $twig, $config, $dataBase)
                {
                    ...
                    {   
                        ...
                        if (empty($paymentRequestParameters['data']['customer']['first_name']) && empty($paymentRequestParameters['data']['customer']['last_name'])) {
                            ...
                            if(in_array($paymentKey, ['NOVALNET_CC', 'NOVALNET_SEPA', 'NOVALNET_INSTALMENT_INVOICE']) || ($paymentKey == 'NOVALNET_PAYPAL' && $oneClickShopping == true) ) {
        ...
        $eventDispatcher->listen(ExecutePayment::class,
            function (ExecutePayment $event) use ($paymentHelper, $paymentService, $sessionStorage)
            {
                if($paymentHelper->getPaymentKeyByMop($event->getMop())) {
                    ...
                    $paymentService->performServerCall($paymentKey);
